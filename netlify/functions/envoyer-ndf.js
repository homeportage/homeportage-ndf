...
exports.handler = async (event) => {
  ...
  const sections = [
    {
      ...
      labels: ['Publicité','Téléphone','Fournitures administratives','Frais annuels / uniques (RCP, TPE...)','Location local','Confort véhicule']
    },
    {
      title: 'FRAIS DU PORTÉ',
      labels: ['Repas','Pressing','Vêtements (1 tenue/an)','Coiffeur']
    },
    {
      title: 'FRAIS DE DÉPLACEMENT',
      labels: ['Avion / Train / Hôtel','Parking / Autoroute / Lavage','Frais réels véhicule']
    }
  ];
  ...
  function buildSection(title, labels) {
    return `
      <tr>
        <td colspan="4" style="background:#e8e8e8;padding:8px 10px;font-size:11px;font-weight:bold;color:#444;letter-spacing:0.8px;">${title}</td>
      </tr>
  ...
  const autresSection = autresHt > 0 ? `
    <tr>
      <td colspan="4" style="background:#e8e8e8;padding:8px 10px;font-size:11px;font-weight:bold;color:#444;letter-spacing:0.8px;">AUTRES</td>
    </tr>
  ...
  const kmSection = `
    <tr>
      <td colspan="4" style="background:#e8e8e8;padding:8px 10px;font-size:11px;font-weight:bold;color:#444;letter-spacing:0.8px;">INDEMNITÉS KILOMÉTRIQUES</td>
    </tr>
    <tr style="background:#ffffff;">
...
<div style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  ...
  <div style="padding:20px;">
    ...
    <!-- TABLEAU DÉTAIL -->
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;border:1px solid #e5e5e5;border-radius:6px;overflow:hidden;">
      <thead>
        <tr style="background:#f0f0f0;">
          <th style="padding:9px 10px;text-align:left;font-size:11px;color:#666;font-weight:600;">LIBELLÉ</th>
          <th style="padding:9px 10px;text-align:right;font-size:11px;color:#666;font-weight:600;">H.T. (€)</th>
          <th style="padding:9px 10px;text-align:right;font-size:11px;color:#666;font-weight:600;">TVA (€)</th>
          <th style="padding:9px 10px;text-align:right;font-size:11px;color:#666;font-weight:600;">T.T.C (€)</th>
        </tr>
      </thead>
      <tbody>
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
      <tr>
        <td style="font-size:13px;font-weight:bold;padding-bottom:8px;">DATE & SIGNATURE :</td>
      </tr>
      <tr>
        <td style="font-size:12px;color:#666;padding-bottom:10px;">Fait le ${today} par ${prenom} ${nom}</td>
      </tr>
      <tr>
        <td>
          <div style="border:2px solid #00B4A0;border-radius:6px;padding:6px;display:inline-block;background:#fafafa;">
            <img src="${signature}" style="display:block;height:90px;max-width:280px;" alt="Signature ${prenom} ${nom}">
          </div>
        </td>
      </tr>
    </table>
